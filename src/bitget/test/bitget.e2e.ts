...
if (!API_KEY || !API_SECRET || !API_PASSPHRASE) {
  throw new Error(
    'Please set BITGET_API_KEY and BITGET_API_SECRET and BITGET_API_PASSPHRASE in .env file'
  );
}
...
describe('Bitget Class Integration Tests', () => {
  beforeAll(() => {
  ...
  describe('getTickers', () => {
    it('should fetch multiple tickers successfully', async () => {
      ...
      const tikers = result._unsafeUnwrap();
      expect(tikers).toHaveLength(symbols.length);
      tikers.forEach(ticker => tickerSchema.parse(ticker));
    });
  ...
  describe('getBalance', () => {
    ...
    it('should filter account balance when symbols are provided', async () => {
      const result = await binance.getBalance({ currencies: ['SUSDT'] });
      // Validate filtered balance against BalanceSchema
      const balance = result._unsafeUnwrap();
      balanceSchema.parse(balance);
      expect(Object.keys(balance.currencies)).toEqual(['SUSDT']);
    });
  });
